...
//   try {
//     if (!req.file) {
//       return res.status(400).json({ error: 'Aucun fichier reçu. Le champ doit être nommé "photo".' });
//     }
  
//     console.log("Fichier reçu :", req.file.path);
//     // console.log("Fichier reçu :", req.file);
//     const filePath = req.file.path;
//     const result = await detectObject(filePath);
//     res.json(result);
//   } catch (err) {
...
//   if (!description) return res.status(400).json({ error: 'Texte manquant' });

//   try {
//     const result = await askOpenAI(description);
//     res.json({ result });
//   } catch (err) {
//     res.status(500).json({ error: 'Erreur analyse texte' });
//   }
// };
...
// exports.analyzeImage = async (req, res) => {
//   try {
//     console.log("📸 Requête reçue pour analyse d'image");
